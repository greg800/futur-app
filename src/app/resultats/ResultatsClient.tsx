...
export default function ResultatsClient({ questions, respondents: initial, responseMap: initialMap, summaries, isAdmin }: Props) {
  ...
  async function deleteResponses(r: Respondent) {
    ...
    if (res.ok) {
      setRespondents(prev => prev.filter(p => p.id !== r.id))
      setResponseMap(prev => {
        const next = { ...prev }
        delete next[r.id]
        return next
  ...
  return (
    <div style={{ padding: '40px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6, flexWrap: 'wrap', gap: 12 }}>
        <h1 style={{ fontSize: 26, fontWeight: 700 }}>Résultats</h1>
        {isAdmin && <AdminBar />}
      </div>
      ...
      <div style={{ overflowX: 'auto', borderRadius: 12, border: '1px solid var(--border)' }}>
        <table style={{
          ...
          <thead>
            ...
            {isAdmin && (
              <tr style={{ background: 'var(--surface)', borderBottom: '2px solid var(--border)' }}>
                <th style={{ ...thLabel(), color: 'var(--text-secondary)', fontWeight: 500 }}></th>
                {respondents.map(r => (
                  <td key={r.id} style={{ ...tdCell(), textAlign: 'center' }}>
                    <button
                      onClick={() => deleteResponses(r)}
            ...
            {!isAdmin && (
              <tr style={{ background: 'var(--surface)', borderBottom: '2px solid var(--border)' }}>
                <th style={thLabel()}></th>
                {respondents.map(r => <td key={r.id} style={tdCell()}></td>)}
              </tr>
            )}
          </thead>
          ...
          <tbody>
            {questions.map((q, idx) => (
              <tr key={q.id} style={{ borderBottom: '1px solid var(--border)', background: idx % 2 === 0 ? '#fff' : 'var(--surface)' }}>
                <td style={{
                  padding: '12px 16px', width: LABEL_W, minWidth: LABEL_W, maxWidth: LABEL_W,
                  verticalAlign: 'middle', borderRight: '1px solid var(--border)',
                }}>
...
function thLabel(): React.CSSProperties {
  return {
    ...
    color: 'var(--text)', width: 260, minWidth: 260,
...
function thCell(): React.CSSProperties {
  return {
    ...
    borderRight: '1px solid var(--border)', minWidth: 160, maxWidth: 160,
  }
}

function tdCell(): React.CSSProperties {
  return { padding: '10px 16px', borderRight: '1px solid var(--border)', verticalAlign: 'middle' }
}
